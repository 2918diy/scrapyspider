...
class Forum_Content_Spider(Spider):
    ...
    def parse_post(self, response):
        ...
        soup.append(BeautifulSoup(question[0]))
        soup.append(BeautifulSoup("<div><p>this is Ron's answers for "+response.url+"</p></div>"))
        ...
        for answers in response.xpath('//div[@id="page-body"]/div[starts-with(@class,"post bg")]'):
            if answers.xpath('./div/dl/dt/a/text()').extract()[0] == 'RonPurewal':
               soup.append(BeautifulSoup(answers.xpath('./div/div/div').extract()[0]))
    ...
    def parse_content_post(self, response):
        ...
        #content = []
        for answers in response.xpath('//div[@id="page-body"]/div[starts-with(@class,"post bg")]'):
            if len(answers.xpath('./div/dl/dt/a/text()').extract())!=0:
                if answers.xpath('./div/dl/dt/a/text()').extract()[0] == 'RonPurewal':
                   soup.append(BeautifulSoup(answers.xpath('./div/div/div').extract()[0]))
